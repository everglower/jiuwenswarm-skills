...
| 平台 | 渲染方式 | 可获取数据 | 搜索方法 |
|------|---------|----------|---------|
| **豆瓣读书** (book.douban.com) | 可直接抓取 | 书名、评分(1-10)、评价人数、作者、出版社、短评、标签、书单 | 网络搜索 搜 "{类型} site:book.douban.com" 拿 URL；或直接抓取标签页 `https://book.douban.com/tag/{标签名}` 提取列表中的书名/评分/出版信息 |
| **Goodreads** (goodreads.com) | 可直接抓取 | 国际评分(1-5)、评分人数、Genre 标签、类似书推荐 | 网络搜索 搜 "site:goodreads.com books similar to {书名}" 或 "best {genre} novels 2024 site:goodreads.com" |
| **知乎** (zhihu.com) | 可直接抓取 | 书单回答帖、类型推荐帖、避雷帖 | 网络搜索 搜 "{类型} 小说 推荐 书荒 site:zhihu.com" |
| **百度贴吧** (tieba.baidu.com) | 可直接抓取 | 网文口碑、类型吧推荐神作帖 | 网络搜索 搜 "{类型} 神作 推荐 site:tieba.baidu.com" |
| **番茄小说** (fanqienovel.com) | 无法直接抓取（页面内容由前端动态加载） | 免费网文排行榜、分类榜单（男频/女频各 20+ 子类型）、热度数据 | 网络搜索 搜 "{类型} site:fanqienovel.com" 拿到书页 URL 后，用 网页内容提取 打开提取书名/简介/评分/字数。直接抓取首页可获取分类列表：西方奇幻/东方仙侠/科幻末世/都市日常/都市修真/都市高武/历史古代/战神赘婿/都市种田/传统玄幻/悬疑灵异/抗战谍战 等 |
| **起点中文网** (qidian.com) | 反爬严格（直接抓取返回空内容） | 网文评分、追人数、月票榜、完结榜 | 必须用 网络搜索 搜 "{类型} site:qidian.com 完结 排行" 拿到页面 URL，再用 网页内容提取 打开。无法直接抓取 |
| **晋江文学城** (jjwxc.net) | 需处理 GBK 编码 | 言情/纯爱类评分、收藏榜、完结文 | 网络搜索 搜 "{类型} site:jjwxc.net 高分 完结"。直接抓取需处理编码 |
| **微信读书** (weread.qq.com) | 可直接抓取 | 出版书评分、读者标记、热门榜单 | 网络搜索 搜 "{类型} site:weread.qq.com 热门" |
| **龙的天空** (lkong.com) | 可直接抓取 | 资深网文书评、神作共识贴 | 网络搜索 搜 "{类型} 神作 site:lkong.com" |

### 搜索工具使用顺序（重要）

1. **第一选择**：`网络搜索` -- 搜索关键词，获取搜索结果（标题+URL+摘要）
2. **第二选择**：`网页内容提取` -- 打开搜索结果中的具体页面 URL，提取完整正文
3. **备选**：`直接抓取 直接抓取（仅适用于豆瓣读书等可直接抓取站点）
4. ⚠️ 起点/番茄小说等 无法直接抓取（前端动态加载或反爬），必须走 网页内容提取
...
- **注意**：起点/番茄小说无法直接抓取，必须用 网页内容提取
- **豆瓣可直接抓取**：豆瓣读书标签页是可直接抓取，可直接用 直接抓取 抓取
- **不剧透**：简介只给设定和开局
